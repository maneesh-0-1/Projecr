...
class FavoritesViewController: UICollectionViewController, UICollectionViewDelegateFlowLayout, FavoriteAddressDialogDelegate {
    ...
    func delete(id: String) {
        Network.shared.apollo.perform(mutation: DeleteAddressMutation(id: id)) { result in
            switch result {
            case .success(_):
    ...
    func update(address: GetAddressesQuery.Data.RiderAddress) {
        ...
        dialog.addAction(UIAlertAction(title: "Done", style: .default){ action in
            Network.shared.apollo.perform(mutation: UpdateAddressMutation(id: address.id, title: vc.textTitle.text ?? "", details: vc.textAddress.text ?? "", lat: vc.map.camera.centerCoordinate.latitude, lng: vc.map.camera.centerCoordinate.longitude)) { result in
                switch result {
                case .success(_):
                    self.refreshList(self)
    ...
    @IBAction func onAddFavoriteClicked(_ sender: Any) {
        ...
        dialog.addAction(UIAlertAction(title: NSLocalizedString("Done", comment: ""), style: .default) { action in
            ...
            Network.shared.apollo.perform(mutation: CreateAddressMutation(title: vc.textTitle.text ?? "", details: vc.textAddress.text ?? "", lat: vc.map.camera.centerCoordinate.latitude, lng: vc.map.camera.centerCoordinate.longitude)) { result in
                switch result {
                case .success(_):
                    self.refreshList(self)
    ...
    override func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        cell.textAddress.text = address.details
