...
class EarningViewController: UIViewController {
    ...
    func refresh(queryType: TimeQuery) {
        Network.shared.apollo.fetch(query: GetStatsQuery(timeFrame: queryType)) { result in
            switch result {
            case .success(let _data):
                guard let response = _data.data?.getStats else {
                    return
                }
                ...
                self.textTime.text = formatter.string(from: current.time)
                self.textEarning.text = MyLocale.formattedCurrency(amount: current.earning, currency: response.currency)
                let distanceFormatter = MKDistanceFormatter()
                distanceFormatter.unitStyle = .abbreviated
                self.textDistance.text = distanceFormatter.string(fromDistance: current.distance)
    ...
    func setEarningChart(dataPoints: [GetStatsQuery.Data.GetStat.Dataset]) {
    ...
    func setDistanceChart(dataPoints: [GetStatsQuery.Data.GetStat.Dataset]) {
        styleChart(chart: distanceChart)
        var dataEntries: [BarChartDataEntry] = []
        for i in 0..<dataPoints.count {
            let dataEntry = BarChartDataEntry(x: Double(i), yValues: [dataPoints[i].distance])
            dataEntries.append(dataEntry)
        }
    ...
    func setTimeChart(dataPoints: [GetStatsQuery.Data.GetStat.Dataset]) {
        styleChart(chart: timeChart)
        var dataEntries: [BarChartDataEntry] = []
        for i in 0..<dataPoints.count {
            let dataEntry = BarChartDataEntry(x: Double(i), yValues: [dataPoints[i].time])
            dataEntries.append(dataEntry)
        }
    ...
    func setCountChart(dataPoints: [GetStatsQuery.Data.GetStat.Dataset]) {
        styleChart(chart: countChart)
        var dataEntries: [BarChartDataEntry] = []
        for i in 0..<dataPoints.count {
            let dataEntry = BarChartDataEntry(x: Double(i), yValues: [dataPoints[i].count])
            dataEntries.append(dataEntry)
        }
    ...
    func getFormatter(dataPoints: [GetStatsQuery.Data.GetStat.Dataset]) -> IndexAxisValueFormatter {
