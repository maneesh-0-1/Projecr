...
class ChargeAccountActivity : BaseActivity() {
    ...
    private var walletItems: List<WalletQuery.RiderWallet> = ArrayList()
        ...
        this.viewModel.wallet.observe(this) {
            when(it) {
                ...
                is ViewState.Success -> {
                    ...
                    if (response.riderWallets.count() < 1) {
                    ...
                    this.walletItems = response.riderWallets.sortedByDescending { x -> x.balance }
                    ...
                    if (response.riderWallets.isNotEmpty()) {
