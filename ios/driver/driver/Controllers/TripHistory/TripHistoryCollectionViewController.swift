...
class TripHistoryCollectionViewController: UICollectionViewController, UICollectionViewDelegateFlowLayout {
    ...
    var travels = [HistoryQuery.Data.Completed.Edge.Node]()
    ...
    @IBAction func refreshList(_ sender: Any) {
        Network.shared.apollo.fetch(query: HistoryQuery()) { result in
            switch result {
            case .success(let data):
                guard let items = data.data?.completed.edges else {
                    return
                }
                self.travels = items.map { item in item.node }
        ...
        return UIContextMenuConfiguration(identifier: nil, previewProvider: nil, actionProvider: { suggestedActions in
            let complaint = UIAction(title: NSLocalizedString("Write Complaint", comment: ""), image: UIImage(systemName: "square.and.pencil")) { action in
                ...
                dialog.addAction(UIAlertAction(title: NSLocalizedString("OK", comment: ""), style: .default) { action in
                    Network.shared.apollo.perform(mutation: WriteComplaintMutation(orderId: self.travels[indexPath.row].id, subject: dialog.textFields![0].text ?? "", content: dialog.textFields![1].text ?? "")) { result in
                        switch result {
                        case .success(_):
...
//            }
    ...
    override func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        cell.textCost.text = MyLocale.formattedCurrency(amount: travel.costAfterCoupon, currency: travel.currency)
        
        cell.textStatus.text = travel.status.rawValue
        return cell
    }
}
