...
class PromotionsCollectionViewController: UICollectionViewController, UICollectionViewDelegateFlowLayout {
    ...
    var promotions = [GetAnnouncementsQuery.Data.Announcement.Edge]()
    var colors = [[String]]()
    ...
    @IBAction func refreshList(_ sender: Any) {
        ...
        Network.shared.apollo.fetch(query: GetAnnouncementsQuery()) { result in
            switch result {
            case .success(let response):
                if let announcements = response.data?.announcements {
                    self.promotions = announcements.edges
